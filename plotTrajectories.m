...
% %%%%%%%%%%%%%%%%%%%%%%%%%%GRAPHICS%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% %If the chosen parss are the same of the article, choose their same color
    ...
    for t = floor(MaxTime/Divider)+1:100:MaxTime-100
        for k = 1:numel(pars)
            if(t+100<=size(Trajectories{k},1))
                if(Ndim == 2)
                    plot([Trajectories{k}(t,1),Trajectories{k}(t+100,1)], [Trajectories{k}(t,2),Trajectories{k}(t+100,2)], "-", color=colors(k), LineWidth=3)
                elseif(Ndim == 3)
                    plot3([Trajectories{k}(t,1),Trajectories{k}(t+100,1)], [Trajectories{k}(t,2),Trajectories{k}(t+100,2)], [Trajectories{k}(t,3),Trajectories{k}(t+100,3)], "-", color=colors(k), LineWidth=3)
                end
            end
            refresh
            drawnow limitrate
